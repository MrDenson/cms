...
class User extends Base
{
    public function __construct()
    {
        ...
        //判断用户登录状态
        $ac = request()->action();
        if (in_array($ac, ['login', 'logout', 'ajax_login', 'reg', 'findpass', 'findpass_msg', 'findpass_reset', 'reg_msg', 'oauth', 'logincallback'])) {
    ...
    public function visit()
    {
        ...
        if($param['uid'] > 0){
            ...
            $info = model('Visit')->infoData($where);
            ...
            if($info['code']>1) {
                ...
                $res = model('User')->where('user_id', $GLOBALS['user']['user_id'])->setInc('user_points',intval($GLOBALS['config']['user']['invite_visit_points']) );
            }
        }
